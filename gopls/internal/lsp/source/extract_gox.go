...
func gopExtractVariable(fset *token.FileSet, start, end token.Pos, src []byte, file *ast.File, pkg *types.Package, info *typesutil.Info) (*analysis.SuggestedFix, error) {
	...
	expr, path, ok, err := GopCanExtractVariable(start, end, file)
	if !ok {
	...
	case *ast.BasicLit, *ast.CompositeLit, *ast.IndexExpr, *ast.SliceExpr,
		*ast.UnaryExpr, *ast.BinaryExpr, *ast.SelectorExpr:
		lhsName, _ := gopGenerateAvailableIdentifier(expr.Pos(), file, path, pkg, info, "x", 0)
	...
	case *ast.CallExpr:
		...
		for i := 0; i < tup.Len(); i++ {
			...
			var lhsName string
			lhsName, idx = gopGenerateAvailableIdentifier(expr.Pos(), file, path, pkg, info, "x", idx)
			lhsNames = append(lhsNames, lhsName)
		}
	default:
...
// gopGenerateAvailableIdentifier adjusts the new function name until there are no collisions in scope.
// Possible collisions include other function and variable names. Returns the next index to check for prefix.
func gopGenerateAvailableIdentifier(pos token.Pos, file *ast.File, path []ast.Node, pkg *types.Package, info *typesutil.Info, prefix string, idx int) (string, int) {
	scopes := GopCollectScopes(info, path, pos)
...
// gopCollectFreeVars maps each identifier in the given range to whether it is "free."
...
func gopCollectFreeVars(info *typesutil.Info, file *ast.File, fileScope, pkgScope *types.Scope, start, end token.Pos, node ast.Node) ([]*variable, error) {
		...
		case *ast.AssignStmt:
			for _, assignment := range n.Lhs {
				...
				for _, expr := range n.Rhs {
					if gopReferencesObj(info, expr, obj) {
						continue
					}
					if _, ok := seen[obj]; !ok {
						continue
...
// gopReferencesObj checks whether the given object appears in the given expression.
func gopReferencesObj(info *typesutil.Info, expr ast.Expr, obj types.Object) bool {
	var hasObj bool
...
// gopObjUsed checks if the object is used within the range. It returns the first
// occurrence of the object in the range, if it exists.
func gopObjUsed(info *typesutil.Info, start, end token.Pos, obj types.Object) (bool, *ast.Ident) {
...
// gopVarOverridden traverses the given AST node until we find the given identifier. Then, we
...
func gopVarOverridden(info *typesutil.Info, firstUse *ast.Ident, obj types.Object, isFree bool, node ast.Node) bool {
...
// gopParseBlockStmt generates an AST file from the given text. We then return the portion of the
...
// gopGenerateReturnInfo generates the information we need to adjust the return statements and
...
func gopGenerateReturnInfo(enclosing *ast.FuncType, pkg *types.Package, path []ast.Node, file *ast.File, info *typesutil.Info, fset *token.FileSet, pos token.Pos, hasNonNestedReturns bool) ([]*gopReturnVariable, *ast.IfStmt, error) {
	var retVars []*gopReturnVariable
	var cond *ast.Ident
	if !hasNonNestedReturns {
		// Generate information for the added bool value.
		name, _ := gopGenerateAvailableIdentifier(pos, file, path, pkg, info, "shouldReturn", 0)
		cond = &ast.Ident{Name: name}
		retVars = append(retVars, &gopReturnVariable{
			name:    cond,
	...
	if enclosing.Results != nil {
		...
		for _, field := range enclosing.Results.List {
			...
			var name string
			name, idx = gopGenerateAvailableIdentifier(pos, file, path, pkg, info, "returnValue", idx)
			retVars = append(retVars, &gopReturnVariable{
	...
	if !hasNonNestedReturns {
		...
		ifReturn = &ast.IfStmt{
			Cond: cond,
			Body: &ast.BlockStmt{
				List: []ast.Stmt{&ast.ReturnStmt{Results: gopGetNames(retVars)[1:]}},
			},
		}
	}
...
// gopAdjustReturnStatements adds "zero values" of the given types to each return statement
// in the given AST node.
func gopAdjustReturnStatements(returnTypes []*ast.Field, seenVars map[types.Object]ast.Expr, fset *token.FileSet, file *ast.File, pkg *types.Package, extractedBlock *ast.BlockStmt) error {
...
// gopGenerateFuncCall constructs a call expression for the extracted function, described by the
// given parameters and return variables.
func gopGenerateFuncCall(hasNonNestedReturn, hasReturnVals bool, params, returns []ast.Expr, name string, token token.Token, selector string) ast.Node {
...
// gopReturnVariable keeps track of the information we need to properly introduce a new variable
// that we will return in the extracted function.
type gopReturnVariable struct {
...
// gopExtractMethod refactors the selected block of code into a new method.
func gopExtractMethod(fset *token.FileSet, start, end token.Pos, src []byte, file *ast.File, pkg *types.Package, info *typesutil.Info) (*analysis.SuggestedFix, error) {
	return gopExtractFunctionMethod(fset, start, end, src, file, pkg, info, true)
}

// gopExtractFunction refactors the selected block of code into a new function.
func gopExtractFunction(fset *token.FileSet, start, end token.Pos, src []byte, file *ast.File, pkg *types.Package, info *typesutil.Info) (*analysis.SuggestedFix, error) {
	return gopExtractFunctionMethod(fset, start, end, src, file, pkg, info, false)
}

// gopExtractFunctionMethod refactors the selected block of code into a new function/method.
...
func gopExtractFunctionMethod(fset *token.FileSet, start, end token.Pos, src []byte, file *ast.File, pkg *types.Package, info *typesutil.Info, isMethod bool) (*analysis.SuggestedFix, error) {
	...
	p, ok, methodOk, err := GopCanExtractFunction(tok, start, end, src, file)
	...
	variables, err := gopCollectFreeVars(info, file, fileScope, pkgScope, start, end, path[0])
	if err != nil {
		return nil, err
	}
	...
	for _, v := range variables {
		...
		isUsed, firstUseAfter := gopObjUsed(info, end, v.obj.Parent().End(), v.obj)
		if v.assigned && isUsed && !gopVarOverridden(info, firstUseAfter, v.obj, v.free, outer) {
	...
	gopReorderParams(params, paramTypes)
	...
	extractedBlock, err := gopParseBlockStmt(fset, selection)
	if err != nil {
		return nil, err
	}
	...
	if containsReturnStatement {
		if !hasNonNestedReturn {
			...
			// signature.
			if err := gopAdjustReturnStatements(returnTypes, seenVars, fset, file,
		...
		// function.
		retVars, ifReturn, err = gopGenerateReturnInfo(enclosing, pkg, path, file, info, fset, start, hasNonNestedReturn)
		if err != nil {
			return nil, err
		}
	}
	...
	if hasReturnValues && !hasNonNestedReturn {
		extractedBlock.List = append(extractedBlock.List, &ast.ReturnStmt{
			Results: append(returns, gopGetZeroVals(retVars)...),
		})
	}
	...
	if isMethod {
		...
		name = "newFunction"
		funName, _ = gopGenerateAvailableIdentifier(start, file, path, pkg, info, name, 0)
	}
	extractedFunCall := gopGenerateFuncCall(hasNonNestedReturn, hasReturnValues, params,
		append(returns, gopGetNames(retVars)...), funName, sym, receiverName)
	...
	newFunc := &ast.FuncDecl{
		...
		Type: &ast.FuncType{
			...
			Results: &ast.FieldList{List: append(returnTypes, gopGetDecls(retVars)...)},
		},
		Body: extractedBlock,
	}
	if isMethod {
	...
	if canDefineCount != len(returns) {
		declarations = gopInitializeVars(uninitialized, retVars, seenUninitialized, seenVars)
	}
...
// gopIsSelector reports if e is the selector expr <x>, <sel>.
func gopIsSelector(e ast.Expr, x, sel string) bool {
...
// gopReorderParams reorders the given parameters in-place to follow common Go conventions.
func gopReorderParams(params []ast.Expr, paramTypes []*ast.Field) {
	// Move Context parameter (if any) to front.
	for i, t := range paramTypes {
		if gopIsSelector(t.Type, "context", "Context") {
...
// gopInitializeVars creates variable declarations, if needed.
...
// var y string
// x, y, z = fn()
func gopInitializeVars(uninitialized []types.Object, retVars []*gopReturnVariable, seenUninitialized map[types.Object]struct{}, seenVars map[types.Object]ast.Expr) []ast.Stmt {
	var declarations []ast.Stmt
...
// gopGetNames returns the names from the given list of returnVariable.
func gopGetNames(retVars []*gopReturnVariable) []ast.Expr {
	var names []ast.Expr
...
// gopGetZeroVals returns the "zero values" from the given list of returnVariable.
func gopGetZeroVals(retVars []*gopReturnVariable) []ast.Expr {
	var zvs []ast.Expr
	for _, retVar := range retVars {
...
// gopGetDecls returns the declarations from the given list of returnVariable.
func gopGetDecls(retVars []*gopReturnVariable) []*ast.Field {
	var decls []*ast.Field
	for _, retVar := range retVars {
