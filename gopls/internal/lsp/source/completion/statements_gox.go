...
func (c *gopCompleter) addStatementCandidates() {
	c.addErrCheck()
	c.addAssignAppend()
}
...
func (c *gopCompleter) addAssignAppend() {
	if len(c.path) < 3 {
		return
	}
	...
	case *ast.AssignStmt:
		...
		exprIdx := gopExprAtPos(c.pos, n.Rhs)
		if exprIdx == len(n.Rhs) || exprIdx > len(n.Lhs)-1 {
			return
		}

		lhsType := c.pkg.GopTypesInfo().TypeOf(n.Lhs[exprIdx])
		if lhsType == nil {
			return
		}
...
// nil is returned.
func (c *gopCompleter) topCandidate() *CompletionItem {
...
func (c *gopCompleter) addErrCheck() {
	...
	var (
		...
		testVar          = gopGetTestVar(c.enclosingFunc, c.pkg)
		isTest           = testVar != ""
	...
	prevLine := gopPrevStmt(c.pos, c.path)
	if prevLine == nil {
		return
	}
	...
	if !types.Identical(c.pkg.GopTypesInfo().TypeOf(lastAssignee), errorType) {
		return
	}
...
// gopGetTestVar checks the function signature's input parameters and returns
...
func gopGetTestVar(enclosingFunc *gopFuncInfo, pkg source.Package) string {
