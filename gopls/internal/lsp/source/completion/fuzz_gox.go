...
func (c *gopCompleter) fuzz(typ types.Type, mset *types.MethodSet, imp *importInfo, cb func(candidate), fset *token.FileSet) bool {
...
Loop:
	...
	if ok && strings.HasPrefix("Fuzz", id.Name) {
		...
		f := func(n ast.Node) bool {
			...
			got := c.pkg.GopTypesInfo().Types[s.X]
		...
		for i, a := range add.Args {
			info := c.pkg.GopTypesInfo().TypeOf(a)
			if info == nil {
