...
// possible.
func (c *gopCompleter) builtinArgKind(ctx context.Context, obj types.Object, call *ast.CallExpr) objKind {
	builtin, err := c.snapshot.BuiltinFile(ctx)
	if err != nil {
		return 0
	}
	exprIdx := gopExprAtPos(c.pos, call.Args)
...
// call's parent node.
func (c *gopCompleter) builtinArgType(obj types.Object, call *ast.CallExpr, parentInf candidateInference) candidateInference {
	var (
		exprIdx = gopExprAtPos(c.pos, call.Args)
	...
	case "append":
		...
		if len(call.Args) > 0 {
			inf.objType = c.pkg.GopTypesInfo().TypeOf(call.Args[0])
		}
		if inf.objType == nil {
			inf.objType = parentInf.objType
		}
		...
		if sliceChain := gopObjChain(c.pkg.GopTypesInfo(), call.Args[0]); len(sliceChain) > 0 {
	...
	case "delete":
		if exprIdx > 0 && len(call.Args) > 0 {
			...
			firstArgType := c.pkg.GopTypesInfo().TypeOf(call.Args[0])
			if firstArgType != nil {
	...
	case "copy":
		...
		if len(call.Args) > 0 {
			t1 = c.pkg.GopTypesInfo().TypeOf(call.Args[0])
			if len(call.Args) > 1 {
				t2 = c.pkg.GopTypesInfo().TypeOf(call.Args[1])
			}
		}
