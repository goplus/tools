...
FindCall:
	...
	if overloads != nil {
		...
		for i, o := range overloads {
			osig := o.Type().(*types.Signature)
			info, doc, err := makeInfoEx(ident.Name, o, osig)
			if err != nil {
				return nil, 0, 0, nil
			}
			if o.Name() == obj.Name() {
			...
			if osig.Variadic() || (osig.Params() != nil && osig.Params().Len() > activeParam) || (osig.Params() == nil && activeParam == 0) {
		...
		log.Println("~~~~~~", name, activeSignature, matchSignature, checkBestSignature(activeSignature, matchSignature), activeParam)
