...
// GopPrepareCallHierarchy returns an array of CallHierarchyItem for a file and the position within the file.
func GopPrepareCallHierarchy(ctx context.Context, snapshot Snapshot, fh FileHandle, pp protocol.Position) ([]protocol.CallHierarchyItem, error) {
	ctx, done := event.Start(ctx, "source.GopPrepareCallHierarchy")
	defer done()
...
// GopIncomingCalls returns an array of CallHierarchyIncomingCall for a file and the position within the file.
func GopIncomingCalls(ctx context.Context, snapshot Snapshot, fh FileHandle, pos protocol.Position) ([]protocol.CallHierarchyIncomingCall, error) {
	ctx, done := event.Start(ctx, "source.GopIncomingCalls")
	defer done()

	refs, err := gopReferences(ctx, snapshot, fh, pos, false)
	if err != nil {
...
// gopEnclosingNodeCallItem creates a CallHierarchyItem representing the function call at loc.
func gopEnclosingNodeCallItem(ctx context.Context, snapshot Snapshot, pkgPath PackagePath, loc protocol.Location) (protocol.CallHierarchyItem, error) {
	...
	_, pgf, err := NarrowestPackageForGopFile(ctx, snapshot, fh.URI())
	if err != nil {
		return protocol.CallHierarchyItem{}, err
	}
	start, end, err := pgf.RangePos(loc.Range)
...
// GopOutgoingCalls returns an array of CallHierarchyOutgoingCall for a file and the position within the file.
func GopOutgoingCalls(ctx context.Context, snapshot Snapshot, fh FileHandle, pp protocol.Position) ([]protocol.CallHierarchyOutgoingCall, error) {
	ctx, done := event.Start(ctx, "source.GopOutgoingCalls")
	defer done()
	...
	declPkg, declPGF, err := NarrowestPackageForGopFile(ctx, snapshot, uri)
	if err != nil {
		return nil, err
	}
	...
	declNode, _, _ := gopFindDeclInfo([]*ast.File{declPGF.File}, declPos)
	if declNode == nil {
	...
	for _, callRange := range callRanges {
		_, obj, _ := gopReferencedObject(declPkg, declPGF, callRange.start)
		if obj == nil {
			continue
		}
