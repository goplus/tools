...
// gopComputeImportEdits computes a set of edits that perform one or all of the
// necessary import fixes.
func gopComputeImportEdits(ctx context.Context, snapshot Snapshot, pgf *ParsedGopFile, options *imports.Options) (allFixEdits []protocol.TextEdit, editsPerFix []*ImportFix, err error) {
	...
	allFixEdits, err = gopComputeFixEdits(snapshot, pgf, options, allFixes)
	if err != nil {
		return nil, nil, err
	}
	...
	for _, fix := range allFixes {
		edits, err := gopComputeFixEdits(snapshot, pgf, options, []*imports.ImportFix{fix})
		if err != nil {
			return nil, nil, err
		}
...
// GopComputeOneImportFixEdits returns text edits for a single import fix.
func GopComputeOneImportFixEdits(snapshot Snapshot, pgf *ParsedGopFile, fix *imports.ImportFix) ([]protocol.TextEdit, error) {
	...
	return gopComputeFixEdits(snapshot, pgf, options, []*imports.ImportFix{fix})
}

func gopComputeFixEdits(snapshot Snapshot, pgf *ParsedGopFile, options *imports.Options, fixes []*imports.ImportFix) ([]protocol.TextEdit, error) {
	// trim the original data to match fixedData
	left, err := gopImportPrefix(pgf.Src)
	if err != nil {
		return nil, err
	}
...
// gopImportPrefix returns the prefix of the given file content through the final
	...
	for _, d := range f.Decls {
		if x, ok := d.(*ast.GenDecl); ok && x.Tok == token.IMPORT {
			if e, err := safetoken.Offset(tok, d.End()); err != nil {
				return "", fmt.Errorf("gopImportPrefix: %s", err)
			} else if e > importEnd {
				importEnd = e
			}
		}
