...
// GopImplementation returns a new sorted array of locations of
...
func GopImplementation(ctx context.Context, snapshot Snapshot, f FileHandle, pp protocol.Position) ([]protocol.Location, error) {
	ctx, done := event.Start(ctx, "source.GopImplementation")
	defer done()

	locs, err := gopImplementations(ctx, snapshot, f, pp)
	if err != nil {
		return nil, err
	}
...
func gopImplementations(ctx context.Context, snapshot Snapshot, fh FileHandle, pp protocol.Position) ([]protocol.Location, error) {
	// Type-check the query package, find the query identifier,
	// and locate the type or method declaration it refers to.
	declPosn, err := gopTypeDeclPosition(ctx, snapshot, fh.URI(), pp)
	if goxls.DbgImplementation {
		log.Println("gopImplementations gopTypeDeclPosition:", declPosn, err)
	}
	if err != nil {
		return nil, err
	}
	...
	declPkg := localPkgs[0]
	declFile, err := declPkg.GopFile(declURI)
	if err != nil {
	...
	path := gopPathEnclosingObjNode(declFile.File, pos)
	if path == nil {
	...
	obj := declPkg.GopTypesInfo().ObjectOf(id) // may be nil
...
// gopTypeDeclPosition returns the position of the declaration of the
// type (or one of its methods) referred to at (uri, ppos).
func gopTypeDeclPosition(ctx context.Context, snapshot Snapshot, uri span.URI, ppos protocol.Position) (token.Position, error) {
...
// gopLocalImplementations searches within pkg for declarations of all
...
func gopLocalImplementations(ctx context.Context, snapshot Snapshot, pkg Package, queryType types.Type, methodID string) ([]protocol.Location, error) {
	queryType = methodsets.EnsurePointer(queryType)

	// Scan through all type declarations in the syntax.
	locs, methodLocs := goLocalImplementations(pkg, queryType, methodID)
	...
	for _, pgf := range pkg.CompiledGopFiles() {
		ast.Inspect(pgf.File, func(n ast.Node) bool {
			...
			def := pkg.GopTypesInfo().Defs[spec.Name]
			if def == nil {
			...
			if methodID == "" {
				// Found matching type.
				locs = append(locs, gopMustLocation(pgf, spec.Name))
				return true
			}
