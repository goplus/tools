...
func TestStaticcheckGenerics(t *testing.T) {
	...
	testenv.SkipAfterGo1Point(t, 22) // staticcheck has issues with generics in Go 1.23+
...
func TestStaticcheckRelatedInfo(t *testing.T) {
	...
	testenv.SkipAfterGo1Point(t, 22) // staticcheck has issues with type assertions in Go 1.23+
	const files = `
-- go.mod --
module mod.test
