...
type Constraint = satisfy.Constraint

// A Finder inspects the type-checked ASTs of Go/Go+ packages and
...
// type-checker.
func (f *Finder) Find(info *types.Info, files []*ast.File, gopInfo *typesutil.Info, gopFiles []*gopast.File) {
	if f.Result == nil {
		f.Result = make(map[Constraint]bool)
	}
	f.gopFind(gopInfo, gopFiles)
